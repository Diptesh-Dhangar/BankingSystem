...
{
JLabel headingJLabel  = new JLabel("Update Account");
	
	JLabel enterAccountNoJLabel = new JLabel("Account No:");
	...
	JTextField enterAccountNoJTextField   = new JTextField();
	...
	public UpdateAccount()
	{
		...
		add(headingJLabel);
		headingJLabel.setBounds(480, 10, 190, 25);
		headingJLabel.setFont(new Font("Arial", Font.BOLD, 25));
		
		add(enterAccountNoJLabel);
		enterAccountNoJLabel.setBounds(850, 10, 120, 20);
		enterAccountNoJLabel.setFont(new Font("Arial", Font.BOLD, 20));
		add(enterAccountNoJTextField);
		enterAccountNoJTextField.setBounds(970, 10, 190, 25);
		enterAccountNoJTextField.setFont(new Font("Arial", Font.BOLD, 25));
		...
		mobileJTextField.setBounds(480, 200, 200, 21);
		mobileJTextField.setFont( new Font("Arial", Font.PLAIN, 17));
		...
		accountBalanceJLabel.setBounds(515, 540, 180, 20);
		accountBalanceJLabel.setFont(new Font("Arial", Font.PLAIN, 23));
		add(accountBalanceJTextField);
		accountBalanceJTextField.setBounds(480, 570, 250, 30);
		...
		add(updateJButton);
		updateJButton.setBounds(900, 640, 200, 35);
		updateJButton.setFont(new Font("Arial", Font.BOLD, 20) );
		updateJButton.setBackground(Color.BLACK);
		...
		viewDetialsJButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				long accountNo = Long.parseLong(enterAccountNoJTextField.getText());
				
					try {
						Connection con = DBConnection.getCon();
						PreparedStatement ps1 = con.prepareStatement("SELECT * FROM CREATEACCOUNT1 WHERE ACCOUNT_NO=?");
						PreparedStatement ps2 = con.prepareStatement("SELECT * FROM CREATEACCOUNT2 WHERE ACCOUNT_NO=?");
						PreparedStatement ps3 = con.prepareStatement("SELECT * FROM CREATEACCOUNT3 WHERE ACCOUNT_NO=?");
						PreparedStatement ps4 = con.prepareStatement("SELECT * FROM BANK WHERE ACCOUNT_NO=?");
						...
						if (rs1.next() && rs2.next() && rs3.next() && rs4.next())
						{
							...
							mobileJTextField.setText((String)rs1.getString(9));
							mailJTextField.setText(rs1.getString(9));
							maritalStatusJTextField.setText(rs1.getString(10));
							addressJTextField.setText(rs1.getString(11));
							cityJTextField.setText(rs1.getString(12));
							pinCodeJTextField.setText(rs1.getString(13));
							stateJTextField.setText(rs1.getString(14));
						    ...
						    accountBalanceJTextField.setText((String)rs4.getString(3)+".-/rs");
						}else
						{
							JOptionPane.showMessageDialog(UpdateAccount.this, "Invalid Account Number"+"\n Please check the account number and re-enter it");
						}
					} catch (Exception e2) {
						e2.printStackTrace();
					}
		...
		updateJButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				long mobileno = Long.parseLong(mobileJTextField.getText());
				String email = mailJTextField.getText();
				String maritalStatus = maritalStatusJTextField.getText();
				...
				String income =incomeJTextField.getText();
				String education = educationJTextField.getText();
				String occupation = occupationJTextField.getText();
				
				try {
					Connection con = DBConnection.getCon();
					PreparedStatement ps1 = con.prepareStatement("Update CREATEACCOUNT1 SET MOBILE=? EMAIL=?,MARITAL=?,ADDRESS=?,CITY=?,PINCODE=?,STATE=? where ACCOUNT_NO=?");
					PreparedStatement ps2 = con.prepareStatement("Update CREATEACCOUNT2 SET INCOME=?,EDUCATION=?,OCCUPATION=? where ACCOUNT_NO=?");
					
					ps1.setLong(1,mobileno);
					ps1.setString(2, email);
					ps1.setString(3, maritalStatus);
					ps1.setString(4, address);
					ps1.setString(5, city);
					ps1.setLong(6, pinCode);
					ps1.setString(7, state);
					ps1.setLong(8, accountNO);
					
					int i = ps1.executeUpdate();
					
					ps2.setString(1, income);
					ps2.setString(2, education);
					ps2.setString(3, occupation);
					ps2.setLong(4, accountNO);
					
					int j = ps2.executeUpdate();
					
					if (i==1 && j==1)
					{
						JOptionPane.showMessageDialog(UpdateAccount.this, "Account Updated Successfully");
					} else {
						JOptionPane.showMessageDialog(UpdateAccount.this,"Sorry...!!!!"+ "\nAccount Updation Failed");
					}
	...
	public static void main(String[] args) {
		...
		updateAccount.setResizable(false);
	}
}
